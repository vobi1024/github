...
void connect() {
        ...
        rst = 0;
        Serial.print("\nconnecting...");
        while (!client.connect("node02", "nexus", "letmein")) {
                Serial.print(".");
                delay(1000);
                rst++;
                if (rst >= 20)
        ...
        client.subscribe("/to/node02/+", 1);

        // client.unsubscribe("/hello");
        client.publish("/from/node02/status", "ONLINE", 0, 1);
}
...
void setup() {
        ...
        client.setWill("/from/node02/status", "OFFLINE", 0, 1);
        connect();
        client.publish("/from/node02/status", "START", 0, 1);
}
